...
<div id="comments"> <?php // див с этим id нужен для якорьных ссылок на комменты ?>
	...
	<div class="comment-pagination">
		<?php $args = array( // аргументы для пагинации
				'prev_text' => '< Пред.', // текст назад
				'next_text' => 'След. >' // текст вперед
			);
	...
	<?php if (comments_open()) { // если комментирование включено для данного поста
		...
		$fields =  array( // разметка текстовых полей формы
			'author' => '<label for="author">Ваше имя <input id="author" name="author" type="text" placeholder="Ирина Александровна" value="'.esc_attr($commenter['comment_author']).'" size="30" required></label>', // поле Имя
			'email' => '<label for="email">Ваш e-mail адрес<input id="email" name="email" type="text" placeholder="youremail@gmail.com" value=""'.esc_attr($commenter['comment_author_email']).'" size="30" required></label>', // поле email
		...
		$args = array( // опции формы комментирования
			...
			'comment_field' => '<label for="comment" class="labelcomment">Ваш комментарий <textarea id="comment" name="comment" placeholder="Введите текст сообщения..." rows="6"></textarea></label>', // разметка поля для комментирования
